...
class VisualCorrespondenceExplainer:
    ...
    def overlay_heatmap(self, image_path,heatmap,filename=None,image_size=None,alpha=0.5,colormap_name=None, return_bytes = False):
        ...
        heatmap_resized = Image.fromarray(((heatmap/heatmap.max())*255).astype(np.uint8)).resize(original_image.size, 4)
        heatmap_resized = np.array(heatmap_resized)
        heatmap_resized = np.stack([heatmap_resized,heatmap_resized,heatmap_resized],-1)
